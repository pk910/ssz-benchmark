#!/usr/bin/env node
/**
 * SSZ Benchmark SVG Chart Generator
 * Generates static SVG images optimized for GitHub README (light and dark mode)
 */
...
function generateChartsSvg(data, colorScheme) {
    ...
    // Build chart data: for each type, for each metric, show all 3 operations in a row
    ...
    // Layout: 3 charts per row, 4 rows (Block Time, Block Memory, State Time, State Memory)
    // Target width: 830px for GitHub README
    ...
    const totalWidth = rowWidth + labelAreaWidth; // ~830px
...
`;
    ...
    svg += `<text x="${totalWidth / 2}" y="24" text-anchor="middle" font-family="system-ui, -apple-system, sans-serif" font-size="18" font-weight="700" fill="${colors.title}">SSZ Library Benchmark Comparison</text>`;
    svg += `<text x="${totalWidth / 2}" y="44" text-anchor="middle" font-family="system-ui, -apple-system, sans-serif" font-size="11" fill="${colors.subtitle}">Mainnet Preset • Latest Stable Versions • Lower is Better • Generated: ${generatedDate}</text>`;
    ...
    charts.forEach((row, rowIdx) => {
        ...
        row.charts.forEach((chart, chartIdx) => {
            ...
            svg += generateVerticalBarChart(chart.title, chart.data, chart.formatFn, rowIdx * 3 + chartIdx, colors);
            svg += `</g>`;
        });
    });
...
function main() {
    ...
    const baseOutputPath = process.argv[2] || path.join(__dirname, '..', 'benchmark-charts.svg');
    ...
    console.log('Generating dark mode SVG charts...');
    ...
    console.log('Generating light mode SVG charts...');
