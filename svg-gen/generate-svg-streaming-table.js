#!/usr/bin/env node
/**
 * SSZ Benchmark SVG Streaming Table Generator
 * Generates a compact table for streaming benchmarks (MarshalWriter, UnmarshalReader)
 * Optimized for GitHub README (light and dark mode)
 * Layout: Libraries as rows, Streaming Operations as columns
 */
...
const OPERATION_LABELS = {
    'UnmarshalReader': 'Unmarshal',
    'MarshalWriter': 'Marshal'
};
...
function generateTableSvg(data, colorScheme) {
    ...
    // Dimensions scaled for GitHub README width (narrower than buffer table)
    ...
    const sectionHeight = typeHeaderHeight + rowsPerSection * cellHeight;
...
`;
    ...
    svg += `<text x="${totalWidth / 2}" y="24" text-anchor="middle" font-family="system-ui, -apple-system, sans-serif" font-size="18" font-weight="700" fill="${colors.title}">SSZ Streaming Benchmark Results</text>`;
    svg += `<text x="${totalWidth / 2}" y="44" text-anchor="middle" font-family="system-ui, -apple-system, sans-serif" font-size="11" fill="${colors.subtitle}">Mainnet • Reader/Writer APIs • Lower is Better • ● = Best • Generated: ${generatedDate}</text>`;
    ...
    OPERATIONS.forEach((op, opIdx) => {
        ...
        const displayLabel = OPERATION_LABELS[op] || op;
        svg += `<rect x="${x}" y="${tableY}" width="${opColWidth * 2}" height="${headerHeight}" fill="${colors.headerBg}" ${isLast ? 'rx="0 6 0 0"' : ''}/>`;
        svg += `<text x="${x + opColWidth}" y="${tableY + 18}" text-anchor="middle" font-family="system-ui, -apple-system, sans-serif" font-size="12" font-weight="600" fill="${colors.headerText}">${displayLabel}</text>`;
...
function main() {
    ...
    const baseOutputPath = process.argv[2] || path.join(__dirname, '..', 'benchmark-streaming-table.svg');
    ...
    console.log('Generating dark mode streaming SVG table...');
    ...
    console.log('Generating light mode streaming SVG table...');
